...
const auth = {
    login(e) {
        ...
        fAuth.signInWithEmailAndPassword(email, pass).catch(err => alert("Erro: " + err.message));
    },
...
const app = {
    ...
    async handleProductSubmit(e) {
        ...
        // Só atualiza a foto se uma nova tiver sido selecionada
    ...
    async processMove(e) {
        ...
        try {
            ...
            // DISPARO DE EMAIL ALERTA
            if (type === 'SAIDA' && newQty <= (pData.minThreshold || 0) && (pData.minThreshold > 0)) {
                emailjs.send(EMAILJS_SERVICE_ID, EMAILJS_TEMPLATE_ID, {
                    product_name: pData.name,
                    current_qty: newQty,
                    min_threshold: pData.minThreshold,
                    to_emails: "gestor@niteroi.com" // Mude para seu e-mail real
                });
            }
            ui.closeModal('move');
    ...
    async showHistory(pid) {
        ...
        try {
            ...
            content.innerHTML = logs.map(l => `
                <div class="log-item">
                    <strong>${l.type} ${l.qty}un</strong> - Setor: ${l.sector} | 
                    <small>Por: ${l.employee}</small>
                </div>
            `).join('') || 'Sem movimentações nos últimos 30 dias.';
        } catch (err) { 
            console.error(err);
            content.innerHTML = "Erro ao carregar log. Verifique se o índice composto foi criado no Firebase."; 
        }
    },
    ...
    async deleteProduct(id) {
        if (confirm("Deseja excluir este produto e todo seu histórico permanentemente?")) {
            try {
...
const ui = {
    ...
    closeModal(id) { 
        ...
        // Reseta o formulário e limpa campos escondidos
        const form = document.querySelector(`#modal-${id} form`);
        if(form) form.reset();
        if(document.getElementById('p-edit-id')) document.getElementById('p-edit-id').value = "";
    },
